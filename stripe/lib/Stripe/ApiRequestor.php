...
class Stripe_ApiRequestor2
{
  ...
  {
    if ($d instanceof Stripe_ApiResource2) {
  ...
  public function handleApiError($rbody, $rcode, $resp)
  {
    if (!is_array($resp) || !isset($resp['error'])) {
      ...
      throw new Stripe_ApiError2($msg, $rcode, $rbody, $resp);
    }
    ...
    case 400:
        if ($code == 'rate_limit') {
          throw new Stripe_RateLimitError2(
    ...
    case 401:
        throw new Stripe_AuthenticationError2($msg, $rcode, $rbody, $resp);
    case 402:
        throw new Stripe_CardError2($msg, $param, $code, $rcode, $rbody, $resp);
    default:
        throw new Stripe_ApiError2($msg, $rcode, $rbody, $resp);
    }
  }
  ...
  private function _requestRaw($method, $url, $params)
  {
    ...
    if (!$myApiKey)
      $myApiKey = Stripe2::$apiKey;

    if (!$myApiKey) {
      $msg = 'No API key provided.  (HINT: set your API key using '
           . '"Stripe2::setApiKey(<API-KEY>)".  You can generate API keys from '
           . 'the Stripe web interface.  See https://stripe.com/api for '
           . 'details, or email support@stripe.com if you have any questions.';
      throw new Stripe_AuthenticationError2($msg);
    }
    ...
    $uname = php_uname();
    $ua = array('bindings_version' => Stripe2::VERSION,
                'lang' => 'php',
                'lang_version' => $langVersion,
                'publisher' => 'stripe',
                'uname' => $uname);
    $headers = array('X-Stripe-Client-User-Agent: ' . json_encode($ua),
                     'User-Agent: Stripe/v1 PhpBindings/' . Stripe2::VERSION,
                     'Authorization: Bearer ' . $myApiKey);
    if (Stripe2::$apiVersion)
      $headers[] = 'Stripe-Version: ' . Stripe2::$apiVersion;
  ...
  private function _interpretResponse($rbody, $rcode)
  {
    try {
      ...
      $msg = "Invalid response body from API: $rbody "
           . "(HTTP response code was $rcode)";
      throw new Stripe_ApiError2($msg, $rcode, $rbody);
    }
  ...
  private function _curlRequest($method, $absUrl, $headers, $params)
  {
    ...
    if ($method == 'get') {
      ...
      throw new Stripe_ApiError2("Unrecognized method $method");
    }
  ...
  public function handleCurlError($errno, $message)
  {
    ...
    throw new Stripe_ApiConnectionError2($msg);
  }
  ...
  private function checkSslCert($url)
  {
    ...
    if ($errno !== 0) {
        throw new Stripe_ApiConnectionError2(
