...
class Stripe_Object2 implements ArrayAccess
{
  /**
  ...
  {
    self::$permanentAttributes = new Stripe_Util_Set2(array('_apiKey', 'id'));
    self::$nestedUpdatableAttributes = new Stripe_Util_Set2(array('metadata'));
  }
  ...
  public function __construct($id=null, $apiKey=null)
  {
    $this->_apiKey = $apiKey;
    $this->_values = array();
    $this->_unsavedValues = new Stripe_Util_Set2();
    $this->_transientValues = new Stripe_Util_Set2();
  ...
  public function refreshFrom($values, $apiKey, $partial=false)
  {
    ...
    foreach ($values as $k => $v) {
      ...
      if (self::$nestedUpdatableAttributes->includes($k) && is_array($v))
        $this->_values[$k] = Stripe_Object2::scopedConstructFrom('Stripe_AttachedObject2', $v, $apiKey);
      else
        $this->_values[$k] = Stripe_Util2::convertToStripeObject($v, $apiKey);
  ...
  public function serializeParameters()
  {
    ...
    foreach (self::$nestedUpdatableAttributes->toArray() as $property) {
      if (isset($this->$property) && $this->$property instanceOf Stripe_Object2) {
  ...
  {
    if ($recursive)
      return Stripe_Util2::convertStripeObjectToArray($this->_values);
    else
      return $this->_values;
  }
}
